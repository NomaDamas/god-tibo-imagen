...
몬스터는 투명 PNG 한 장과 공용 모션 프로필로 움직인다. `MONSTER LAB`에서 제작 완료된 100종의 대기·이동·공격·피격·사망 상태를 구역과 계열별로 확인할 수 있다.
...
완성된 중앙 광장, 기록 도서관, 체육 실습관, 응용 과학동, 온실 정원은 구역마다 다섯 원형을 기초형·강화형·오류형·초월형으로 확장한 100종이다. 이후 구역도 `monster-production.json`에 제작 원형을 추가하면 같은 계약으로 검증과 전시 범위가 함께 확장된다.
